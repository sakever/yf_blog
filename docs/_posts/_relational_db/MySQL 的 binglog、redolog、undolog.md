...
## binlog 日志
binlog（归档日志，又叫二进制日志）：mysql 数据库自带的日志（mysql 分两层，上层是 mysql-server 服务层，下层是可替换的数据库引擎，比如 innodb 等等），值得一提的是，binlog 是默认关闭的
### binlog 的作用
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3a6c0f1ead15782cb2ab15a4813f329b.png)
在每次写入操作的时候，将信息以二进制的形式保存在磁盘中，binlog 是 mysql 的逻辑日志

binlog 可以通过 binlog_format 来控制存储格式，下面三种也叫 MySQL binlog 日志的三种模式：

- row 基于行的复制：binlog 会记录每一行数据的实际变化，即哪些行被插入、更新或删除。从库接收到这些变化后，直接对相应的数据行进行操作。消耗空间大
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b1ebb92b79eb4e6cb255f574c92d32a1.png)

- statement 基于语句的复制：binlog 会记录每一条对数据库进行修改的 SQL 语句，所消耗的空间比较少。这种模式某些函数（如 UUID、NOW）可能导致主从不一致。MySQL 5.7.7 及之前版本默认是 STATEMENT 模式，5.7.7 后默认是 ROW 模式了
- mixed 混合模式复制：会根据情况来自动决定存储格式是 row 还是 statement，不推荐使用。由于是自动选择模式，会给 DBA 带来一些理解和管理上的困难
...
从库有一个 IO 线程专门用来读主库的 event，读到后会生成 **relay log（中继日志）**，从库还有一个 SQL thread，SQL thread 用来在从库上执行 relay log
### 半同步和全同步
由于异步复制的过程，它在主机挂掉的时候，会有一部分已经提交的事务无法复制到从库。半同步相对异步复制来说，能更少的避免数据丢失，不过性能方面较差，下面额外说一下半同步和全同步：

1，半同步是在事务提交之前，将 binlog 发送到从库，然后从库给主库发送确认接受消息，之后主库才进行提交。这个可以通过更改 MySQL 配置来实现的
```mysql
-- 需同时配置：
...
看起来很简单，但是如果加上 mysql 自生机器的二阶段提交，事情就会变的复杂了。之前的半同步流程大概是这样的
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/824a135f06d244c98246b969dee21c5d.png)
本机写 binlog 和从库写 binlog 一起执行，这样就会导致如果主库在发送 binlog 后但未收到确认前崩溃，可能存在数据丢失

因此**增强半同步复制**做了优化，增强半同步是 MySQL 5.7.2后的版本对半同步复制做的一个改进，原理上几乎是一样的，主要是解决幻读的问题。主库配置了参数 rpl_semi_sync_master_wait_point = AFTER_SYNC 后，主库在存储引擎提交事务前，必须先收到从库数据同步完成的确认信息后，才能提交事务，以此来解决该问题
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f0c242336bcf47019767624fa531eb92.png)

2，**全同步则是主库在确认所有的从库接受到消息之后，主库才会提交**，这么做会最大程度上解决主从延迟问题，但是数据库的写入性能也很差。因此一主一从，主写从读全同步，是小型项目中较为不错的数据库集群方案。MySQL 可以基于 Group replication（**组复制**）等实现全同步技术

组复制是一项新技术，在此之前我先说一下，如果有三个从库我设置rpl_semi_sync_master_wait_for_slave_count = 3 是不是就是全同步了？答案是否定的，这只是通过半同步机制强制等待所有从库确认，**主库只确认从库收到了 binlog，不确认从库执行完了事务**（因为半同步会生成 relaylog，让从库异步执行）。底层仍然是主从复制架构，没有分布式一致性保证

组复制中，我们使用 Paxos 协议变种实现分布式共识，实现了自动多数派决策，部分节点可故障，同时还内置自动故障转移和选主功能。工作原理为客户端向主节点发送事务请求，主节点在本地执行事务，但不立即提交，通过 Group Communication System 广播到整个组，等待大多数节点确认接收（N/2+1），收到足够确认后提交本地事务，并且广播提交记录

3，在异步复制中，主库（Master）在将事务提交到本地存储后，不等待从库（Slave）的确认就返回给客户端，这样可能导致数据不一致的问题
### 主从延迟
主从延迟一般出现在读后写或者写后读的场景，比如同一个线程在写库之后立即读取，然后使用主写从读的方案，这时候有可能出现数据读不到的问题。再比如一个线程在写入数据之前需要确认数据库中有没有这个数据，因此先查了一遍数据库，但是因为数据库有延迟，其他的线程对 DB 的修改没有被这个线程感知到，因此出现了修改丢失的问题

处理这个问题，我们作为业务方有以下几种方案：

1，写数据后加2秒分布式锁，这2s内读请求都不允许访问 db，2s后数据肯定同步完毕了
2，如果是单机循环写入并且查询的情况，可以封装写入方法，直到从库有数据才返回
3，对于一些核心业务，可以强制查主库
4，保存写入数据到 redis 或者内存中，下一次查询时拼接到查询结果中
## redolog 日志
...
双1设置：始终设置 innodb_flush_log_at_trx_commit=1，启用二进制日志记录并设置 sync_binlog=1，这就是 MySQL 最安全的设置方法
...
最简单的 redolog 里面存储着表空间 ID、页号、偏移量以及需要更新的值，因此所需内存是比较小的
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d5c5bdcf2f951aabf5ce0d537e8c06b0.png)
- type ：该条 redo 日志的类型
- space ID ：表空间 ID
- page number ：页号
...
这些比较复杂的重写日志类似下面这样
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5b92084486f2e8490707403091e35ade.png)
#### redolog 的完整性
...
一条 sql 语句（比如 insert）可能会操作多棵 B+ 树，所以一条 sql 语句会包含多个 mtr

一个事务可能包含多条 sql 语句。

所以，事务、sql 语句、mtr、redo log 的关系如下：
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cee5b91c57aaafd590ed7d40df260042.png)
### redolog 在磁盘里的结构
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b62eda5f997a28fa8007f758ca8e4e12.png)
...
1，MySQL 内部定义了一个全局变量，每当有事务需要 id 时，则将当前的值分配给该事务，并进行自增1

2，当这个变量值为256的倍数时，则刷新到系统表空间中页号为5的页面下的一个名为 max trx id 的属性中，该属性占用8字节的空间
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d1ae69cb1bfd0a0fdd5708ce29cd17d1.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/4af12223e12ad809c1f216ec564f8a93.png)
...
因此，一个事务的 undolog 不是由页为单位来管理的，而是由链表来管理的，那应该需要一个整合的地方来管理这些链表吧，我们提出了**回滚段**的概念。回滚段就是被称为 Rollback Segment Header 的页面，这个页面中存放了各个 Undo 页面链表的 first undo page 的页号，这些页号被称为 undo slot。这样我们就可以提供 slot 来找到对应的链表头了
...
### undolog 的作用
1，MySQL 就是通过 undolog 回滚日志来保证事务原子性的，准确的说**是再发生错误时提供回滚服务**。在异常发生时，对已经执行的操作进行回滚，回滚日志会先于数据持久化到磁盘上（因为它记录的数据比较少，所以持久化的速度快），当用户需要回滚数据时（比如服务器断电了，再次启动数据库的时候，或者事务中出现异常需要回滚时），数据库能够通过查询回滚日志来回滚将之前未完成的事务

2，**支持 MVCC（多版本并发控制），提高事务读取数据的效率**

**为什么有了 redolog 还需要 undolog？**这就要从磁盘性能的优化说起

Commit Logging 有一个巨大的先天缺陷：所有对数据的真实修改都必须发生在事务提交以后，即日志写入了 Commit Record 之后。在此之前，即使磁盘有足够空闲，即使某个事务修改的数据量非常庞大，占用了大量的内存缓冲区，无论何种理由，都**决不允许在事务提交之前就修改磁盘上的数据**，这一点是 Commit Logging 成立的前提（**因为我们我们不能使用 redolog 删除错误的记录，只能用它重做正确的记录**），却对提升数据库的性能十分不利。为此，ARIES 提出了提前写入日志（Write-Ahead Logging）的日志改进方案，所谓提前写入（Write-Ahead），就是允许在事务提交之前写入变动数据的意思
...
也就是说，MySQL 可能会在事务提交之前刷入脏页，同时也记录了 undo log 用于回滚
### 事务分配 undolog 的过程
...
同时 binlog 也会同步其他机器，mysql 可以设置全同步，那全同步的实现，就是二阶段提交 2PC 了
### 两阶段提交
...
1.**开始事务**：MySQL 客户端发起一个事务，执行一系列的 SQL 操作（如 INSERT、UPDATE、DELETE 等）。此时会持续写入 undo log 和 redo log
...
1.**写入 Binlog**：如果全同步的情况下，参与者都返回了 yes，MySQL 服务器将事务的修改操作记录到 Binlog 中并且刷盘。Binlog 记录了事务的所有 SQL 操作，以便用于主从复制和数据恢复
2. **提交事务**：MySQL 服务器通知 InnoDB 存储引擎提交事务。InnoDB 将事务从 prepare 状态转变为 commit 状态，并记录一个 redo log 的 commit 日志记录。此时，事务的所有修改正式生效
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/0f3a0e6fe8894ecd935bfd3a27a46533.png)
崩溃恢复阶段
...
在 mysql 集群中，事实上是使用异步复制、半同步或者全同步来保证集群数据一致性的。这三者的实现都有所不同