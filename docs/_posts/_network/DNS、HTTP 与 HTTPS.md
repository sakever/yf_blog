...
session：存放在服务器，用于配合 cookie 获取用户状态，session 一般存放在缓存或者数据库中，一般在 cookie 中存放 sessionID，这个 sessionID 可以唯一标识一个用户以此让系统知道该连接对应哪个用户

cookie：存放在用户端，用于记录用户信息或者服务器返回的数据，这些数据可以让系统了解该用户状态

token：有些系统会返回一个 token 存放在 cookie 中，token 是服务端生成的一串字符串，以作客户端进行请求的一个令牌，用户只要带上这个令牌就无需再次带上用户名和密码。token 一般可以使用 JWT 等技术生成
...
目前在分布式场景下，项目都是集群部署的，因此出现了分布式 session 问题，即登录状态等会话数据分散在不同机器内存里，互相看不见，可能会出现用户一会儿登录一会儿又没登录问题，为了处理该问题，我们会使用：

- 1，利用 redis 等分布式中间件构建用户中心，将 session 数据存放在 redis 中，每个用户登录后会生成一个 sessionID，将这个 sessionID 存放在 cookie 中返回给用户，用户后续请求会带上这个 cookie 去用户中心获取登录状态
- 2，利用 token：服务端无状态，只需要校验 Token（token 根据某些加签算法生成），不存 Session
- 3，Session 粘滞（Sticky Session）：Nginx 用 ip_hash 等方式，让同一个 IP 总是打到同一台机器。如果负载不均、那台挂了就丢 Session，一般不用这种方案
### HTTP2.0 和3.0的多路复用
