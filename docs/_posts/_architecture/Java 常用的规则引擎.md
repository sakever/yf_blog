...
- 二是注重动态配置表达式，算子比较简单的框架，他们的特点是做了将动态脚本嵌入 java 的操作，代表为 AviatorScript、Groovy、easy rules 等
...
对比其他的规则引擎，比如 Drools、easy rules，他们要么代码臃肿、难以上手，要么设计过于简单，不支持多场景下的流程编排，因此我们选用 LiteFlow，它代码落地简单、**性能优秀（官网有压测报告）、支持多场景、支持热部署能力**
