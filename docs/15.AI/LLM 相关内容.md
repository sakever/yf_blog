...
目前大模型大部分是 Decoder only 结构，因为 decoder-only 结构模型在没有任何微调数据的情况下，zero-shot 的表现能力最好，由于其因果掩码的机制天然适合预测下一个词的概率。而 encoder-decoder 则需要在一定量的标注数据上做 multitask-finetuning 才能够激发最佳性能
### 涌现能力
...
训练时，我们是把完整的目标句子一次性喂给解码器的，比如目标句“I have a cat”。

如果允许自注意力看到整个句子，那在预测“have”的时候，它已经看到了后面的“a”和“cat”，这就等于作弊，模型学会的是抄答案，而不是真正的生成能力

所以，在训练时，必须人为给“未来”的词蒙上眼睛。推理时，我们本来就是一个个词往外蹦，后面词确实不存在，但为了和训练时的行为严格一致，我们同样会把还没生成的位置遮住

因此掩码自注意力是为了在训练时模拟推理时的只能看到过去的约束，强迫模型学会预测下一个词

具体实现起来也非常简单，在未来位置，把注意力分数压到负无穷，让 softmax 输出的权重变成 0

上文说大部分模型都只用到了 Decoder，在训练时，整个目标句子是错位输入的，这样训练完毕后的 KQV，就比较适合用来输出下一个可能单词的向量了，这个向量不仅仅是包含了过去句子的信息，同时也可以做相似度匹配，匹配下一个出现的新向量

训练时，我们用错位输入+掩码，推理时，我们每次输入当前已有的序列，取最后一个位置的输出，重复同样的预测动作，然后把这个新词拼回去，继续下一步。两个模式在数学上完全一致，因为掩码保证了它们都是**因果自回归**的形式
...
针对大模型（LLM）的调用，缓存通常指两种不同层面的技术。绝大多数情况下，算法侧讨论的是 KV Cache（键值缓存），这是大模型能够快起来的核心命脉。另一种则是语义 Prompt 缓存，用于避免重复计算，工程侧同学经常接触

KV Cache（推理加速层面的缓存）是大模型生成文本时必须使用的底层优化技术，解决了重复计算和算力浪费的问题。这是典型的以显存换时间，随着对话变长，缓存占用的显存会线性增长

大模型生成文字是一个字一个字蹦出来的（自回归）。当生成第100个词时，模型需要前99个词的 KV 才能预测下一个。如果没有缓存，每生成一个新词，模型都要把前99个词的注意力全部重新算一遍

推理时生成第一个词（Prefill 阶段）时，算出所有词的 K 和 V，存入显存中的缓存区。生成第二个词（Decode 阶段）时，只计算第二个词的 K 和 V，然后把它和缓存中第一个词的 K、V 拼接起来做注意力计算

语义/Prompt 缓存是指云服务商（如 OpenAI、DeepSeek 等）在 API 网关层面做的缓存。解决了相同或相似问题反复花钱和耗时的问题

如果大量用户都在问“介绍一下量子计算”，或者你的程序用完全相同的系统提示词（System Prompt）调用无数次，每次都走完整模型计算，既费钱（算力成本）又费时

原理实现有两种

- 精确匹配：将完整的 Prompt（提示词）作为 Key，模型生成的回复作为 Value，存入高速缓存（如 Redis）。下一次请求完全一致时，直接返回缓存结果，延迟从秒级降到毫秒级，成本降为0。不过这种和 mysql 的缓存差不多比较鸡肋
- 前缀缓存（语义近似）：更高级的实现中，系统会提取 Prompt 的语义向量（Embedding）。如果发现新 Prompt 与缓存中的某个 Prompt 向量距离很近（意思相同），则复用该结果，或者复用模型计算过程中前几层的 KV Cache（因为前几层提取的是通用语法特征，后几层才是个性化语义）

也就是说，在 Transformer 的数学计算层面，系统提示词和用户提示词的处理流程完全一样，但是在工程缓存层面，存在巨大的性能差异。系统提示词通常是固定的（如“你是一个乐于助人的 AI...”）。在工程上（如 vLLM、SGLang 等框架），系统提示词的 KV Cache 会被预先计算并永久驻留显存（prefill 阶段，系统提示词是并行计算的，一次性算出所有层的 KV Cache）。当你发起多次对话时，系统提示词部分完全不需要重新计算，直接复用缓存。这意味着，无论你的系统提示词有多长（比如2000字），它只会在第一次对话时消耗计算资源，后续请求中它几乎是零成本的
