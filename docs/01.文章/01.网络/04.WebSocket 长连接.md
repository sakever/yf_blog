...
那有同学要问了，WebSocket 和 TCP 的功能不是差不多，为什么要包一层。事实上从收发数据这个最原始的功能来看，它俩确实差不多，都是双向传字节流。但是，WebSocket 这一层包装，根本不是为后端与后端之间通信设计的，而是为浏览器（前端）设计的。如果忽略浏览器这个前提，在其他场景下这两者确实差不多。为什么要包一层 WebSocket：

- 浏览器没有裸 TCP 的 API。因为安全策略，浏览器只允许发 HTTP/HTTPS 请求（即 http:// 或 https:// 协议）。它不允许网页代码直接创建一条 tcp:// 连接去操作任意端口，这叫同源策略，具体介绍请看 CORS 跨域资源关系章节
- WebSocket 定义了一套帧（Frame）协议，处理了拆包粘包问题
- 它复用了 HTTP 的 80 和 443 端口。对于运维来说，这就是一个普通的 Web 流量，同时复用了 HTTPS 加密证书
## Java 中实现 webSocket
