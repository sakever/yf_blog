...
### ForkJoinPool
ForkJoinPool 不是为了替代 ExecutorService，而是它的补充，在某些应用场景下性能比 ExecutorService 更好。ForkJoinPool 最开始主要用于实现分而治之的场景，需要程序员去将任务拆分，它天然适合任务可以被快速拆分且子任务间相互独立的场景

关于工作窃取（Work-Stealing）机制的底层实现是 ForkJoinPool 性能碾压 ThreadPoolExecutor 的核心机密，ThreadPoolExecutor 是所有线程共享一个任务队列，高并发下抢任务会产生激烈的锁竞争（CAS 失败重试）。而 ForkJoinPool 给每个工作线程都配备了一个双端队列（Deque），在分治场景下，它会这么工作：

每个线程执行自己队列里的任务。当它 fork() 出一个子任务时，新任务被推入（push）某个线程队列的底部（Bottom）。线程执行任务时，从底部弹出（pop）任务。因为分治任务中，越晚产生的子任务通常粒度越细，LIFO（后进先出） 能减少栈深度，提高缓存局部性

当一个线程发现自己的队列空了，它不会闲着，它会随机选择另一个线程，去偷取（Steal）那个线程队列头部（Top）的任务

因为被偷的线程自己正在底部疯狂地 LIFO 处理最新任务（细粒度任务）。而队列头部存放的是最早入队的、粒度最大的任务。小偷把大任务偷走，可以自己再拆分成子任务，这样既避免了和原线程争抢底部的热数据，又能平衡负载

关于线程安全性，可以分两步看：

1，底部（Bottom）：只有拥有者线程自己能操作（push/pop），所以完全不需要加锁
2，头部（Top）：会被其他小偷线程操作（poll），这里使用 CAS（比较并交换） 原子操作来保证线程安全

### 关于虚拟线程池
java21 中引入了虚拟线程，用同步的方式获得了异步的性能。因此我额外在最后说一下这个

如果想要使用虚拟线程，在 springMVC 项目中，直接配置 spring.threads.virtual.enabled=true 即可，这样 Web 容器底层的线程池会使用虚拟线程，同时 @Async 异步任务、Spring MVC 的异步请求处理等也会替换为虚拟线程执行，对于外部调用方（如浏览器、前端 App、其他微服务）来说，请求和获取结果的方式完全没变，依然是同步阻塞的，因此老项目切换虚拟线程是非常快速并且无损的

但是不建议将虚拟线程池与传统的池化概念（如 corePoolSize、maxPoolSize）混用。虚拟线程的设计初衷是每个任务一个新线程，非常轻量，创建开销极小，因此不需要像平台线程那样进行池化来复用。为虚拟线程设置池大小（如 server.tomcat.threads.max）通常是无效或反效果的，同时如果你在 Service 里手动创建了一个平台线程池（比如 Executors.newFixedThreadPool(10)），然后把任务提交给这个池子，并用 future.get() 同步等待结果
```java
...
public String handle() {
    ...
    // 这里是同步等待，主虚拟线程被阻塞，等待平台线程的结果
    return future.get(); 
}
```
这种情况，在这个 handle 中，会用尽那10个宝贵的平台线程，导致系统吞吐量下降。因此如果你想在内部开启子任务并同步等待结果，请务必让子任务也跑在虚拟线程上，或者干脆不要自定义线程池，直接在主方法里顺序执行（因为虚拟线程挂起几乎不消耗资源）
