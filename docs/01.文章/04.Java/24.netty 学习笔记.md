...
tags: 
  - netty
permalink: /pages/a141f5/
---
 
先讲一些基础知识，从 IO 开始说起：IO 就是计算机内部与外部进行数据传输的过程，比如网络 IO 与磁盘 IO

所有 IO 都需要系统调用，由操作系统代理执行，并经历从 IO 设备拷贝到内核空间拷到用户空间的环节（java 中有管理堆外内存的类，是个特例）
...
#### java 里的 BIO
java 中的 BIO 如下，也是传统的 IO 编程。每一个客户端连接都使用一个线程，在这个连接不传输信息的时候也会占用线程，此外所有调用都是阻塞的
```java
        try {
...
// 优点：一个线程管理成千上万个连接！
```
java 中的 NIO 通信是 IO 多路复用的一种实现，reactor 模式是其一种实现的思想。使用选择器，缓冲区、通道来实现，通道将用户数据拷贝到缓冲区中，选择器让程序读取缓冲区中数据

java 中一个线程对应一个选择器，一个选择器对应多个连接（通道），选择器选择那个通道是由事件确定的。Buffer 是一个内存块，底层用数组实现，要么是输入状态要么是输出状态，每一个通道都要注册到选择器中，选择器检查是否有事件发生来进行通道的选择
#### Reactor 模式
...
## 异步非阻塞 I/O
异步非阻塞 I/O 是一种高效的 I/O 模型，它允许程序在等待 I/O 操作完成时不被阻塞，而是继续执行其他任务。这种方式特别适合处理大量并发连接的场景，因为它可以显著减少线程的数量和上下文切换的开销
...
有聪明的同学就要问了，上文说 netty 底层基于反应器模式，反应器模式是 NIO 的经典落地。现在又说 netty 是异步 IO，那到底那种是对的？

事实上两者都是对的，站在操作系统内核层面，Netty 是同步非阻塞 I/O。站在开发者编程模型层面，Netty 是异步非阻塞 I/O：

- 操作系统层面：判断同步还是异步的唯一标准是数据从内核空间拷贝到用户空间（即 read() 系统调用）的这个动作，是由谁完成的？需不需要当前进程/线程主动参与？是则为同步。因为这个主动调用 read() 拷贝数据的动作是 Netty 的线程亲手执行的，因此 Netty 属于同步非阻塞 I/O（netty 中有零拷贝可以减少拷贝次数，但是如果数据是从网卡读进来，要交给你的业务代码（比如解析 HTTP 请求体），零拷贝无法消除内核态到用户态的拷贝，netty 是将数据从内核复制到直接内存，避免复制到堆中）
- 开发者编程层面：异步指的是应用程序主线程（业务线程）与网络 I/O 事件的执行是否解耦。而 netty 发起 I/O 请求和处理 I/O 结果是完全分离的，由 EventLoop 异步调度。因此，业界习惯称 Netty 为异步非阻塞框架
...
同时 netty 使用了**零拷贝**技术做了优化，减少了上下文切换和数据复制的次数。说了这么多你只需要记住 netty 最大的优点是它解耦了连接和工作线程，异步了请求和响应，用少量线程管理了大量连接
## 典型应用
netty 是高性能基石，很多高性能的框架都是基于 netty 做的。需要处理高并发连接（如万级 TCP 长连接）、低延迟、高吞吐量的网络服务推荐使用

典型应用：

- SocketIO：实时推送（如股票行情、在线游戏）
