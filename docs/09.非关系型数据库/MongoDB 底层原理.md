...
tags:
  - MongoDB
---
MongoDB 是一个基于分布式文件存储的开源 NoSQL 数据库系统，由 C++ 编写的。MongoDB 提供了 面向文档 的存储方式，操作起来比较简单和容易，支持无模式的数据建模，可以存储比较复杂的数据类型，是一款非常流行的文档类型数据库
...
MongoDB 将数据记录存储为文档 （更具体来说是 BSON 文档），这些文档在集合中聚集在一起，数据库中存储一个或多个文档集合
## BSON
...
文档的键是字符串。除了少数例外情况，键可以使用任意 UTF-8 字符，有以下要求：
...
- **WiredTiger 存储引擎** ：自 MongoDB 3.2 以后，默认的存储引擎为 WiredTiger 存储引擎（这是个开源的、面向多核和大内存的键值存储引擎，它本来不是 MongoDB 专属，而是一个可嵌入的 NoSQL 引擎）。非常适合大多数工作负载，建议用于新部署。WiredTiger 提供文档级并发模型、检查点和数据压缩等功能
- In-Memory 存储引擎 ：In-Memory 存储引擎在 MongoDB Enterprise 中可用。它不是将文档存储在磁盘上，而是将它们保留在内存中以获得更可预测的数据延迟，行为模式类似 redis

此外，MongoDB 3.0 提供了可插拔的存储引擎 API ，允许第三方为 MongoDB 开发存储引擎，这点和 MySQL 也比较类似
...
WiredTiger 存储引擎支持 ACID，支持 read-uncommitted 、read-committed 和 snapshot 这3种事务隔离级别，MongoDB 启动时默认选择 snapshot 隔离

其底层原理和 Mysql 类似，使用日志、MVCC、写锁来支持事务，其落库时机等等都差不多，接下类简单介绍一下：

WiredTiger 的持久性机制大致是检查点+预写日志，检查点是指定期将数据以一致状态刷盘，作为一个恢复点。Journal（预写日志）是指检查点之间，所有写操作都会记录到 journal。你看预写日志就类似 redolog，而恢复点就是 binlog

崩溃恢复流程就是从上一个检查点恢复一个一致状态。从该点开始重放 journal 中已提交的事务。默认 journal 每 100ms 同步一次，所以最坏情况可能丢失最近这 100ms 内的提交（可通过写关注 {j: true} 强制每次写入都等待日志持久化）

多版本并发控制 MVCC 和 mysql 基本上一模一样，读不阻塞写，写也不阻塞读；通过维护多版本数据，实现高并发。更新时通常写一个新版本，而不是原地覆盖，这样读事务可以继续读旧版本
## 索引
...
MongoDB 用 B+ 树（更准确地说是 B+ 树变种 B- 数）作为主数据结构，是因为它在读多写少、范围查询多、磁盘/缓存随机访问的场景下，在性能、复杂度和通用性之间，是一个足够好、又足够通用的折中方案。首先 B+ 数的优点大家都知道，数矮、有序、可根据索引查找。在 WiredTiger 中：

- 每个集合是一个 B+ 树，key 是内部 RecordId（8 字节整数），value 是文档 BSON
- 每个索引：也是一个独立的 B+ 树，key 是索引字段值 + RecordId，value 一般是空（或只有少量字段）。通过索引找到 RecordId，再到集合 B+ 树里用 RecordId 取文档
## 分布式文件存储
MongoDB 的分布式能力主要来自分片集群（Sharded Cluster） 架构，它由三个核心组件协同工作，将数据自动切分、路由和冗余存储

1，分片（Shard）：存储数据子集。每个分片是一个副本集，提供冗余和高可用。通常至少 3 个节点，一主两从，主节点宕机时，从节点自动选举新主。并且可做读写分离
2，配置服务器（Config Server）：存储集群元数据，记录哪些数据块在哪个分片上，以及分片键范围等信息
3，查询路由器（mongos）：路由客户端请求，根据配置服务器中的元数据，将读写请求路由到正确的分片

我们要向一个表中存放一条数据时，MongoDB 会先选择分片键，然后数据会按分片键范围或哈希决定数据会存放到哪个块中，平衡器监控各分片负载，自动迁移块，保持均衡。在查询的时候，客户端连接 mongos，mongos 根据分片键将请求转发到对应分片。应用无需关心数据在哪个分片上，mongos 会自动完成路由，对应用透明

MongoDB 的单文档存储空间要求不超过 16MB。对于超过 16MB 的大文件，MongoDB 提供了 GridFS 来进行存储，通过 GridFS，可以将大型数据进行分块处理，然后将这些切分后的小文档保存在数据库中

具体来说，GridFS 将大文件切分成多个较小的块（chunk）（默认 255KB），分别存储为独立的文档，并使用两个集合来管理。集合一是 chunks，是文件二进制数据块。集合二是 files，存放文件元数据。读取文件时驱动程序会按 files_id 和序号 n 查询所有块，并重新组装成完整文件
